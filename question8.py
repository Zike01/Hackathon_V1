...
square 1 to square 100. Players take alternative turns, and move between 1

and 6 spaces based on a random roll of the dice. If they land on a square that ends in 5 (e.g. 5,
...
def player_1(name1):
    ...
    if player1_score % 5 == 0 and player1_score % 2 != 0:
        print(
            f"You've rolled {dice_roll} and you landed on square {player1_score}. "
            f"You've landed on a ladder! You're on square {player1_score + 9} now")
        player1_score += 9
    elif player1_score % 10 == 0:
        print(
            f"You've rolled {dice_roll} and you landed on square {player1_score}. "
            f"You landed on a snake! Player 1 is back at the starting square")
        player1_score = 0
    else:
...
def player_2(name2):
    ...
    if player2_score % 5 == 0 and player2_score % 2 != 0:
        print(
            f"You've rolled {dice_roll} and you landed on square {player2_score}. "
            f"You landed on a ladder! You're on square {player2_score + 9} now")
        player2_score += 9
    elif player2_score % 10 == 0:
        print(
            f"You've rolled {dice_roll} and you landed on square {player2_score}. "
            f"You landed on a snake! Player 2 is back at the starting square")
        player2_score = 0
    else:
